...
extern "C" {
	...
	AAPIEXPORT void * AKIActionCloseLaneDetailedAction(int sectionId, int alane, int VehType, bool apply2LanesCF );
	AAPIEXPORT void * AKIActionAddNextTurningODAction(int sectionId, int anextSection, int aOrigin, int aDest, int VehType, int asectionInPath, double acomplianceLevel);
	...
	AAPIEXPORT void * AKIActionAddNextSubPathODAction(int ang_sectionId, int nbNextSections, int * anextSections, int aOrigin, int aDest, int VehType, int asectionInPath, double acomplianceLevel);
	...
	AAPIEXPORT void * AKIActionAddCloseTurningODAction(int sectionId, int anewSection2Close, int aOrigin, int aDest, int VehType, double acomplianceLevel, bool localEffect = true);
	...
	struct AAPIEXPORT StaticInfVeh{
		...
		//! distance that the vehicle keeps between itself and the preceding vehicle (meters or feet, depending on the units defined in the network).
		...
		//!  0 means vehicle will not change path en-route, 1 means vehicle will change path en-route depending on the percentege of enrouted vehicles defined.
		int enrouted;
		...
		int centroidDest;
		//! Identifier of exit section destination of the vehicle, when the destination centroid uses percentages as destination (otherwise is –1) and the traffic conditions are defined by an O/D matrix.
		int idsectionExit;
	...
	AAPIEXPORT int				AKIVehGetVehTypePosition(const unsigned short *aname);
	...
	AAPIEXPORT structA2KDetector AKIDetGetPropertiesDetectorANG(int nbdet);
	AAPIEXPORT structA2KDetector AKIDetGetPropertiesDetectorANGById(int iddet);
	...
	struct AAPIEXPORT A2KSectionInf{
		...
		double speedLimit;
		//! visibility distance of the section (meters or feet).
		...
		//! length of the section (meters or feet).
		double length;
	...
	*/
	struct AAPIEXPORT StructAkiEstadSystem{
		...
		//!  Flow  (veh/h)
		int Flow;	/* Flow */
		//! Average travel time(seconds/km or seconds/mile)
		double TTa;
		//! Deviation travel time(seconds/km or seconds/mile)
		double TTd;
		//! Average delay time (seconds/km or seconds/mile)
		double DTa;
		//! Deviation delay time (seconds/km or seconds/mile)
		double DTd;
		...
		double Density;
		//! Average stop time (seconds/km or seconds/mile)
		double STa;
		//! Deviation stop time (seconds/km or seconds/mile)
		double STd;
  ...
  */
	...
	AAPIEXPORT int AKIGenerateIncident(int asection, int alane, double position, double length, double initime, double duration);
	...
	AAPIEXPORT int AKIODDemandGetNumSlicesOD();
	AAPIEXPORT double AKIODDemandGetIniTimeSlice(int numslice);
	AAPIEXPORT double AKIODDemandGetEndTimeSlice(int numslice);
	AAPIEXPORT double AKIODDemandGetDemandODPair(int origen, int desti, int VehType, int numslice);
	AAPIEXPORT int AKIODDemandSetDemandODPair(int origen, int desti, int VehType, int numslice, double anewdemand);
	...
	AAPIEXPORT int AKIPastCostSetPastCost(int sectorig, int sectdest, double aTime, const unsigned short *TypeName, double acost, double aocost);
	AAPIEXPORT double AKIPastCostGetPastCost(int sectorig, int sectdest, double aTime, const unsigned short *TypeName);
	AAPIEXPORT double AKIPastCostGetPastOutputCost(int sectorig, int sectdest, double aTime, const unsigned short *TypeName);
	...
	AAPIEXPORT int AKIFleetIsFleetVehicleGenerated(int idFleet, const unsigned short *nameFleetVeh);
